...
// ОПТИМИЗИРОВАННАЯ ВЕРСИЯ: Быстрая проверка для ускорения badarg теста
// Проверка MAXVA идет ПЕРВОЙ для максимальной скорости
static inline int
uaddr_in_range(struct proc *p, uint64 uva, uint64 len)
{
  // ОПТИМИЗАЦИЯ: Проверка MAXVA ПЕРВОЙ (самая быстрая проверка)
  // Это ловит 0xffffffff (badarg тест) мгновенно
  if (uva >= MAXVA)
    return 0;
  ...
  // Проверка на адреса в kernel space
  if (uva >= KERNBASE)
    return 0;
  
  // Для адресов < MAXVA пусть copyin/copyout сами проверят через page table
