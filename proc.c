...
uaddr_in_range(struct proc *p, uint64 uva, uint64 len)
{
  ...
  // Это быстрая проверка для отсева заведомо невалидных адресов
  if (uva >= MAXVA)
    return 0;
  if (len > 0 && uva + len > MAXVA)
    return 0;
  
  // ОПТИМИЗАЦИЯ ДЛЯ ТЕСТА BADARG: быстрая проверка на адреса в верхней половине 32-битного пространства
  ...
  // Вместо медленной проверки через page table, сразу отсеиваем такие адреса
  if (uva >= 0x80000000ULL && uva < MAXVA) {
    return 0;
  }
  
  // Для остальных адресов < MAXVA пусть copyin/copyout сами проверят через page table
  // Это безопасно, так как они делают полную проверку валидности
  // Не проверяем p->sz, так как это может блокировать валидные случаи
  return 1;
}
