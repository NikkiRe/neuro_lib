...
uaddr_in_range(struct proc *p, uint64 uva, uint64 len)
{
  ...
  // ОПТИМИЗАЦИЯ ДЛЯ ТЕСТА BADARG: быстрая проверка на явно невалидные адреса
  ...
  // Также проверяем конкретно на 0xffffffff для еще более быстрой проверки
  if (uva >= 0x80000000ULL || uva == 0xffffffffULL) {
    return 0;
  }

  uint64 sz = p->sz;          // граница пользовательского адресного пространства
